...
* Authoring is done using Cubic Splines (only) even though TTF is quadratic only. We let FF figure out the conversion.
...
* Digit glyphs (0 to 9) should be Cap-height high (and not overshoot).
* When centering non-symmetric glyphs horizontally, use judgement to have roughly equal whitespace area on the right and left. Symmetric glyphs can use FF's "Center in Width".
* Fewer Spine CVs are better.
* When "Generating Fonts" with TrueType selected, validation should be enbled and clean.
* Version starts at 0.1.0. Increment as desired until all ISO8859-1 glyphs are done at which point version is 1.0.0
